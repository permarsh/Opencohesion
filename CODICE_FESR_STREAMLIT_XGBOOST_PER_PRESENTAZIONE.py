...
# Cache the training function to avoid retraining on every interaction
@st.cache_data(show_spinner=True)
def train_model(df, selected_columns, target_var):
    df_filtered = df.copy()
    ...
    if target_var == 'AVANZAMENTO FISICO':
        target_col = '%AVANZ. FISICO '
        df_filtered = df_filtered[(df_filtered[target_col] >= 0) & (df_filtered[target_col] <= 0.99)]
        df_filtered = df_filtered[df_filtered[target_col].notna()]
        df_filtered = df_filtered[df_filtered[target_col] != 'n/d']
        df_filtered[target_col] = pd.to_numeric(df_filtered[target_col], errors='coerce')
        df_filtered = df_filtered[df_filtered[target_col].notna()]

        scaler = MinMaxScaler()
        df_filtered['target_normalized'] = scaler.fit_transform(df_filtered[target_col].values.reshape(-1, 1))
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        class_majority = le_target.transform([le_target.classes_[-1]])[0]
    ...
    return {
        ...
        "accuracy_score": accuracy_score(oof_true, y_pred),
        "f1_macro": f1_score(oof_true, y_pred, average='macro'),
        "classification_report": classification_report(oof_true, y_pred, target_names=le_target.classes_, output_dict=True),
        "confusion_matrix": confusion_matrix(oof_true, y_pred),
        "target_display_name": target_var,
        "le_target_classes": le_target.classes_
    }
...
if st.sidebar.button("Run Model Training and Evaluation"):
    ...
    with st.spinner("Preprocessing data and training model..."):
        results = train_model(df, selected_columns, target_var)

        # Display feature importance
        st.subheader("Top 20 Average Feature Importance")
        fig_fi, ax_fi = plt.subplots(figsize=(5, 4))
        sns.barplot(x=results["avg_importance"].values, y=results["avg_importance"].index, ax=ax_fi)
        ...
        st.subheader(f"Classification Metrics for target: {results['target_display_name']} (Cross-Validation)")
        st.write(f"**Accuracy:** {results['accuracy_score']:.4f}")
        st.write(f"**Macro F1-score:** {results['f1_macro']:.4f}")
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                    xticklabels=results["le_target_classes"], yticklabels=results["le_target_classes"], ax=ax_cm)
